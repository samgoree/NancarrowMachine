...
# Generates a melody with notes pitches, which is returned as a 2d list of distances above C4, with one list per octave of the melody
# TODO
def GenerateMelody(talea):
	retval = []
	for m in range(len(talea)):
		retval.append([])
		for i in range(len(talea[m])):
			retval[m].append(12 * (len(talea) - m))
	return retval
